...
const user = require('../../controllers/v2/user');
const auth = require('../../middleware/auth');
const upload = require('../../controllers/v2/upload')
...
const schema = require('../../helpers/v2/joi-validation');
...
router.post('/loginWithToken', validateSchema(schema.validateUserLoginToken), HOD(user.loginWithToken));
router.get('/getPlansAndPricing', auth.required, common.checkTokenAndStatus, HOD(user.getPlansAndPricing));
router.get('/resendOtp', HOD(user.resendOTP));
...
router.post('/verifyOtp', validateSchema(schema.OTPrequired), HOD(user.verifyOTP));
...
router.post('/sendOtpEmailRegister', validateSchema(schema.email_validate2), HOD(user.sendOtpEmailRegister));
...
router.get('/resendEmailVerificationLink', HOD(user.resendEmailVerificationLink));
router.get('/getUserProfile', auth.required, common.checkTokenAndStatus, HOD(user.getUserProfile));
router.get('/sendCodeToEmail', HOD(user.sendCodeToEmail));
...
router.post('/webHookData', HOD(user.webHookData));
router.post('/webHookBatchTrxData', HOD(user.webHookBatchTrxData));
...
router.post('/updateAndMakePayment', auth.required, common.checkTokenAndStatus, validateSchema(schema.emptySchema), HOD(user.updateAndMakePayment));
...
router.get('/getBookingDetail', auth.required, common.checkTokenAndStatus, validateSchemaGet(schema.getBookingDetailSchema), HOD(user.getBookingDetail));
router.get('/check_users_booking', auth.required, common.checkTokenAndStatus, HOD(user.check_users_booking));
router.get('/check_users_confirmed_booking', auth.required, common.checkTokenAndStatus, HOD(user.check_users_confirmed_booking));
router.get('/configVariables', auth.required, common.checkTokenAndStatus, HOD(user.configVariables));
...
router.post('/AddContactUs', validateSchema(schema.AddContactUs), HOD(user.AddContactUs));//auth.optional, common.checkTokenAndStatus,
...
router.get('/emailInvoice', auth.required, common.checkTokenAndStatus, validateSchemaGet(schema.emailInvoiceSchema), HOD(user.emailInvoice));
...
router.post('/uploadMultipleFiles', s3upload.any('files'), HOD(user.uploadMultipleFiles));
router.post('/uploadAnyChatFiles', s3uploadchat.any('files'), HOD(user.uploadAnyChatFiles));
router.post('/uploadMultipleChatFiles', s3uploadchat.any('files'), HOD(user.uploadMultipleChatFiles));
...
router.post('/updateDeviceToken', auth.required, common.checkTokenAndStatus, validateSchema(schema.updateDeviceToken), HOD(user.updateDeviceToken));
router.post('/closeHomeCard', auth.required, common.checkTokenAndStatus, validateSchema(schema.closeHomeCard), HOD(user.closeHomeCard));
router.get('/viewAllSavedLocation', auth.required, common.checkTokenAndStatus, validateSchemaGet(schema.emptySchema), HOD(user.viewAllSavedLocation));


//router.get('/inviteLinkExpire', HOD(user.inviteLinkExpire));
