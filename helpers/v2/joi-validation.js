...
const schema = {
  ...
  validateUserLogin: Joi.object().keys({
    ...
    link_code: Joi.string().empty('').optional()
  }).or('email', 'phone'),
  ...
  email_validate2: Joi.object().keys({
    email: Joi.string().email({ tlds: { allow: false } }).required(),
    checkRegType: Joi.string().required(),
    randomString: Joi.string().required()
  }),
  emailInvoiceSchema: Joi.object().keys({
  ...
  validate_edit_seat: Joi.object().keys({
    ...
    guest_user: Joi.boolean().empty('').optional()
  ...
  AddContactUs: Joi.object().keys({
    ...
    ipv4: Joi.string().empty('').optional(),
    ipv6: Joi.string().empty('').optional(),
  ...
  validate_getVeriffResponse: Joi.object().keys({
    ...
    verify_timeout: Joi.boolean().default(false).optional()
  }),
  ...
  updateDeviceToken: Joi.object().keys({
    firebase_device_token: Joi.string().required(),
    timezone: Joi.string().empty('').optional()
  }),
  ...
  getBookingDetailSchema: Joi.object({
    bookingId: Joi.string().required()
  }),

  closeHomeCard: Joi.object({
    other_id: Joi.string().required(),
    type: Joi.string().required()
  })
